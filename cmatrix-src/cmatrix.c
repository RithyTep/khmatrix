...
void usage(void) {
    printf(" Usage: cmatrix -[abBcfhlsmVxkK] [-u delay] [-C color] [-t tty] [-M message]\n");
    ...
    printf(" -K: Use Khmer characters. Requires Khmer font support\n");
...
int main(int argc, char *argv[]) {
    ...
    opterr = 0;
    while ((optchr = getopt(argc, argv, "abBcfhlLnrosmxkKVM:u:C:t:")) != EOF) {
        switch (optchr) {
        case 's':
            screensaver = 1;
            break;
...
if (console) {
    ...
    if (khmer) {
        /* Khmer Unicode characters: consonants and digits */
        randmin = 0x1780;  /* ក */
        highnum = 0x17A3;  /* អ */
    } else if (classic) {
